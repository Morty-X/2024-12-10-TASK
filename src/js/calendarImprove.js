(function (window, document) {
  ...
  let curYear = currentDateInstance.getFullYear();
  let curMonth = currentDateInstance.getMonth() + 1;
  ...
  // 把显示和隐藏日历卡片封装到函数中
  window.hideCalendar = function hideCalendar() {
  ...
  let choiceDate = document.querySelector(".date-picker-choiceDate");
  // choiceDate.addEventListener("focus", function () {
  //   initCalendar();
  // });
  ...
  calendar.addEventListener("click", function (e) {
    e.stopPropagation();
    if (e.target.className === "calendar-date__current") {
      let item = e.target.customCalendarInfo;
      console.log(item);
      choiceDate.value = `${item.year}/${item.month}/${item.day}`;
      hideCalendar();
    }
  });
})(window, document);
